                    ...
                    <div class="text-center">
                      ...
                      <form class="user" @submit.prevent="Save">
                        ...
                        <div class="form-group">
                          ...
                          <textarea class="form-control form-control-user textarea" v-model="Payload.content" placeholder="Nhập nội dung tin"  rows="15">
                          </textarea>
                        </div>
...
export default {
  ...
  methods:{
    onDelete(id){
      this.$swal.fire({
        ...
        if (result.isConfirmed) {
           this.$request.delete(`https://localhost:7265/api/news?id=${id}`).then(res=>{
            if(res.status=='200'){
               this.$swal.fire(
                'Đã xoá thành công!',
                '',
    ...
    GetAll(keyword,pageindex,pagesize){
      if(keyword.length >=1){
         this.$request.get(`https://localhost:7265/api/news?keyword=${keyword}&page=${pageindex}&size=${pagesize}`).then(res=>{
          this.Page=res.data
      })
      }
      else{
         
           this.$request.get(`https://localhost:7265/api/news?page=${pageindex}&size=${pagesize}`).then(res=>{
        this.Page=res.data
              console.log(keyword)
        return
      })
